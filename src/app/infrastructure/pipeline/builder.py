...
class PipelineBuilder:
    ...
    def _build_nodes(self, branches: tuple[SourceBranchSpec, ...]) -> tuple[PipelineNodeSpec, ...]:
        ...
        nodes: list[PipelineNodeSpec] = [
            ...
            PipelineNodeSpec(
                ...
                "track",
                {
                    "ll-config-file": str(ds.tracker_config_path),
                },
                {"required": True, "live": True, "supports_probe": True, "hardware_accelerated": True},
            ),
            PipelineNodeSpec(
    ...
    def _build_links(self, branches: tuple[SourceBranchSpec, ...]) -> tuple[tuple[str, str], ...]:
        ...
        links.extend(
            [
                ...
                ("primary-infer", "tracker"),
                ("tracker", "osd"),
                ("osd", "post-osd-queue"),
                ("post-osd-queue", "encoder"),
                ("encoder", "rtmp-pay"),
                ("rtmp-pay", "sink"),
            ]
        )
    ...
    def link_static_elements(self, runtime: dict[str, Any]) -> tuple[tuple[str, str], ...]:
        ...
        request_sources = {req["source"] for req in runtime.get("streammux_requests", ())}
        for source_name, target_name in static_links:
            if source_name in dynamic_sources or source_name in request_sources:
                continue
    ...
    def _batch_meta_to_payload(self, batch_meta: object) -> dict[str, Any]:
        ...
        for frame_meta in self._iterate_glist(frame_list):
    ...
    def _frame_meta_to_payload(self, frame_meta: object) -> dict[str, Any]:
        ...
        for obj_meta in self._iterate_glist(obj_list):
            frame["obj_meta_list"].append(self._object_meta_to_payload(obj_meta))
        return frame
    ...
    def _iterate_glist(self, node: object):
        current = node
        while current is not None:
            data = getattr(current, "data", None)
            if data is not None:
                yield self._cast_meta(data)
            current = getattr(current, "next", None)

    def _cast_meta(self, value: object) -> object:
        if self._runtime_factory.pyds is None or value is None:
            return value
        for cls_name in ("NvDsFrameMeta", "NvDsObjectMeta"):
            cls = getattr(self._runtime_factory.pyds, cls_name, None)
