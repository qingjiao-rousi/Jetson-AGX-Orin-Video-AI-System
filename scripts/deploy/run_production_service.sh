...
cleanup() {
    ...
    if [ "$START_SIMULATOR" = "1" ]; then
        RUNTIME_DIR="$RUNTIME_ROOT/mediamtx_sim" scripts/rtsp/simulate_cameras.sh --stop 2>/dev/null || true
    fi
}
...
if [ "$START_SIMULATOR" = "1" ]; then
    ...
    scripts/rtsp/simulate_cameras.sh "$VIDEO_DIR" --limit "$SOURCE_COUNT"
    sleep 3
fi
...
scripts/rtsp/run_rtsp_inproc.sh "$SERVICE_OUTPUT_DIR"
