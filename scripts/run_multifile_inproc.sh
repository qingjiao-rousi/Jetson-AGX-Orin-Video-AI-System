...
usage() {
    cat <<'USAGE'
Usage:
  scripts/run_multifile_inproc.sh INPUT_VIDEO_DIR [OUTPUT_DIR]
...
python3 scripts/summarize_multifile_inproc.py \
    "$OUTPUT_DIR" \
    "$SUMMARY_PATH" \
    --expected-stream-count "$SOURCE_COUNT"

quality_exit=0
python3 scripts/check_multifile_inproc_outputs.py \
    "$SUMMARY_PATH" \
