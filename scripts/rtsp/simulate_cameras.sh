...
usage() {
    cat <<'EOF'
Usage:
  scripts/rtsp/simulate_cameras.sh VIDEO_DIR [--limit N]    start MediaMTX + FFmpeg camera simulator
  scripts/rtsp/simulate_cameras.sh --stop                    stop simulator
  scripts/rtsp/simulate_cameras.sh --status                  print source_status.json
...
Examples:
  scripts/rtsp/simulate_cameras.sh "$VIDEO_DIR"
  scripts/rtsp/simulate_cameras.sh "$VIDEO_DIR" --limit 4
  scripts/rtsp/simulate_cameras.sh --status
  scripts/rtsp/simulate_cameras.sh --stop
EOF
}
...
if [ "${1:-}" = "--stop" ]; then
    python3 scripts/rtsp/manage_mediamtx_sim.py stop --runtime-dir "$RUNTIME_DIR"
    exit 0
fi

if [ "${1:-}" = "--status" ]; then
    python3 scripts/rtsp/manage_mediamtx_sim.py status --runtime-dir "$RUNTIME_DIR"
    exit 0
fi
...
python3 scripts/rtsp/manage_mediamtx_sim.py start "$VIDEO_DIR" \
