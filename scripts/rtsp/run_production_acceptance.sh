...
usage() {
    cat <<'USAGE'
Usage:
  scripts/rtsp/run_production_acceptance.sh [OUTPUT_DIR]
...
Example:
  RUN_SECONDS=40 START_UI=1 scripts/rtsp/run_production_acceptance.sh
USAGE
}
...
scripts/rtsp/run_rtsp_acceptance.sh "$VIDEO_DIR" "$OUTPUT_DIR"
...
if [ "$ENABLE_INDIVIDUAL_OUTPUTS" = "1" ]; then
    ...
    RUN_SECONDS="$RUN_SECONDS" \
    scripts/rtsp/run_rtsp_individual_outputs.sh \
        "$VIDEO_DIR" \
