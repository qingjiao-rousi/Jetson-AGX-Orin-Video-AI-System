...
usage() {
    cat <<'USAGE'
Usage:
  scripts/rtsp/run_rtsp_individual_outputs.sh [INPUT_VIDEO_DIR] [OUTPUT_DIR]
...
Examples:
  scripts/rtsp/run_rtsp_individual_outputs.sh "$VIDEO_DIR"

  SOURCE_COUNT=8 INDIVIDUAL_JOBS=2 RUN_SECONDS=40 \
    scripts/rtsp/run_rtsp_individual_outputs.sh "$VIDEO_DIR" outputs/rtsp_individual_8
USAGE
}
...
run_one() {
    ...
    scripts/rtsp/run_rtsp_inproc.sh "$stream_dir"
