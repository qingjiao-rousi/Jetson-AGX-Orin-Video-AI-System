...
LEGACY_DIR = Path(__file__).resolve().parents[1] / "legacy" / "person_analytics"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize one in-process RTSP pipeline output directory.")
    parser.add_argument("output_dir", type=Path, help="Output directory from run_rtsp_inproc.sh.")
    parser.add_argument(
        "output_json",
        type=Path,
        nargs="?",
        help="Output summary JSON path. Defaults to rtsp_summary.json in output_dir.",
    )
...
def main() -> int:
    args = parse_args()
    output_json = args.output_json or (args.output_dir / "rtsp_summary.json")
    summary = summarize_rtsp(
        args.output_dir,
...
def summarize_rtsp(
    ...
    *,
    ...
    metrics_jsonl = output_dir / "runtime_metrics.jsonl"
    tiled_video = output_dir / "rtsp_preview.mp4"
    run_log = output_dir / "run.log"
    run_metadata = output_dir / "run_metadata.json"
    runtime_config = output_dir / ".runtime" / "app_rtsp_runtime.yaml"
    source_status_path = output_dir / "source_status.json"
    recovery_check = output_dir / "rtsp_recovery_check.json"

    metadata = _read_json_or_empty(run_metadata)
    metrics_tail = _read_jsonl_tail(metrics_jsonl)
    ...
    return {
        ...
        "recovery_check": str(recovery_check),
        "rtsp_base": metadata.get("rtsp_base", "") if metadata else "",
        "run_seconds": metadata.get("run_seconds") if metadata else None,
        "output_sink": metadata.get("output_sink", "unknown") if metadata else "unknown",
        ...
        "source_status_payload": _read_json_or_empty(source_status_path),
        "runtime_metrics_tail": metrics_tail,
        "processing_fps": metrics_tail.get("processing_fps"),
        "gpu": metrics_tail.get("gpu") or {},
