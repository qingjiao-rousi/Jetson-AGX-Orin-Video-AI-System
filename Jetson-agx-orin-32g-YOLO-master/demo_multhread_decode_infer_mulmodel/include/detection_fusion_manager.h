...
struct FusionConfig
{
    ...
    std::map<int, float> model_weights = {
        // 模型权重
        {0, 1.0}, // 人员检测
        {1, 0.9}, // 安全帽检测
        {2, 0.8}, // 疲劳检测
...
public:
    ...
    struct FusionStats
    {
        int total_detections = 0;      // 总检测框数
        int fused_detections = 0;      // 融合后的检测框数
        int suppressed_detections = 0; // 被抑制的检测框数
        double fusion_time_ms = 0.0;   // 融合时间（毫秒）
    };
