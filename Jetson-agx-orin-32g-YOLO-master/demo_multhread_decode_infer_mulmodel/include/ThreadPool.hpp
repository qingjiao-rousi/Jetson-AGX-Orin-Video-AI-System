...
namespace dpool
{
    ...
    {
    public:
        using MutexGuard = std::lock_guard<std::mutex>;  // 简单加锁
        using UniqueLock = std::unique_lock<std::mutex>; // 可配合条件变量的锁
        using Thread = std::thread;                      // 线程
        ...
        void waitForAll()
        {
            ...
            cv_.wait(lock, [this]()
                     { return tasks_.empty() && (idleThreads_ == currentThreads_); });
        }
