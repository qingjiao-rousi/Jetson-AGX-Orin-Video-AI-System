...
StreamingManager::StreamingManager()
    : streaming_active_(false), should_stop_(false), avformat_context_(nullptr), mpp_encoder_(nullptr), rtmp_frame_index_(0), frame_count_(0)
{
}
...
{                                                   // 把新的一帧合成图像加入推流队列，等待推流线程消费
    std::lock_guard<std::mutex> lock(queue_mutex_); // 先加锁，确保线程安全（push 数据/pop 数据两个访问）

    // 限制队列大小，避免内存溢出
    if (streaming_queue_.size() > 10)
    {
        streaming_queue_.pop();  // 丢弃最旧的数据
        stats_.frames_dropped++; // 统计丢弃的帧数
    }

    streaming_queue_.push(data); // 把新的一帧合成图像加入推流队列
...
{
    if (streaming_active_.load()) // 启动推流工作线程
    {
        return;
    }
...
{
    if (!streaming_active_.load()) // 停止推流工作线程
    {
        return;
    }

    should_stop_ = true; // 不能只设置标志位，还要唤醒可能正在等待条件变量的线程，否则线程可能一直睡着，导致无法退出。
    queue_cv_.notify_all();

    if (streaming_thread_.joinable()) // 等待推流线程结束
    {
...
void StreamingManager::streamingWorker()
{
    ...
    { // 原子读取should_stop_，确保线程安全访问
        StreamingData data;
        bool got_data = false;

        // 等待数据，使用超时机制防止连接超时
        {
            std::unique_lock<std::mutex> lock(queue_mutex_);              // 先加锁，确保线程安全（pop 数据访问）
        ...
        {
            cv::resize(frame, frame, cv::Size(config_.width, config_.height)); // 原图。目标图and尺寸。插值方法（默认线性）
        }
        ...
        { // 创建独立作用域，精细控制锁的持有时间
            ...
            frame_count_++;
            if (frame_count_ % config_.fps == 0) // 每fps算一次帧率，监控推流质量
            {
...
std::string StreamingManager::createDetectionJSON(const StreamingData &data)
{
    ...
    json << "\"timestamp\":" << std::chrono::duration_cast<std::chrono::milliseconds>(data.timestamp.time_since_epoch()).count() << ",";
...
bool StreamingManager::initializeRTMP() // 初始化 RTMP 推流环境，包括 FFmpeg 输出上下文、视频流参数、MPP 编码器和输出连接。
{
    // 初始化 FFmpeg 网络
    ...
    AVCodecParameters *codecpar = stream->codecpar; // 获取视频流参数指针
