...
const int anchor0[6] = {10, 13, 16, 30, 33, 23}; // K-Means 聚类的算法自动计算
const int anchor1[6] = {30, 61, 62, 45, 59, 119};
const int anchor2[6] = {116, 90, 156, 198, 373, 326};

inline static int clamp(float val, int min, int max) { return val > min ? (val < max ? val : max) : min; }

char *readLine(FILE *fp, char *buffer, int *len) // 从文件读取一行字符串，并动态分配内存保存
{
    int ch;
    int i = 0;
    size_t buff_len = 0;
...
{
    int validCount = 0;                                        // 有效框的数量，初始化为零
    int grid_len = grid_h * grid_w;                            // 网格长度，网格高度乘以网格宽度
    int8_t thres_i8 = qnt_f32_to_affine(threshold, zp, scale); // 将一个浮点数阈值（比如 0.5）量化成与模型输出同类型的 INT8 整数
    for (int a = 0; a < 3; a++)                                // 每个尺度有三个anchor
    {
        for (int i = 0; i < grid_h; i++) // 遍历网格高度
        {
            for (int j = 0; j < grid_w; j++) // 遍历网格宽度
            {
                int8_t box_confidence = input[((5 + class_num) * a + 4) * grid_len + i * grid_w + j]; // 相对位置(第a个锚框的置信度) * 总网格数 + 第(i,j)个网格单元的位置
                if (box_confidence >= thres_i8)
                {
                    int offset = ((5 + class_num) * a) * grid_len + i * grid_w + j; // 相对位置(第a个锚框的坐标) * 总网格数 + 第(i,j)个网格单元的位置
                    int8_t *in_ptr = input + offset;                                // 输入数据指针，指向第a个锚框的坐标
