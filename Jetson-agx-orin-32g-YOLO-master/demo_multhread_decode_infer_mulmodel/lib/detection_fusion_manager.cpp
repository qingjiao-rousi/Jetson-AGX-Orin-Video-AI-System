...
#include <iostream>
#include <chrono>
// 检测结果融合：开发检测结果融合模块，基于 IoU 和加权策略对多模型检测框进行匹配、去重和对齐。
    ...
    const detect_result_group_t &callplay_results)
{
    ...
    std::vector<FusedDetection> fused_detections; // 融合后的检测框
    ...
    std::vector<std::vector<bool>> used(4); // 用于标记每个检测框是否已参与融合，防止重复融合相同框对
    for (int i = 0; i < 4; i++)
    {
        used[i].resize(model_detections[i].size(), false); // 初始化每个模型的检测框标记为未融合
    }
...
std::vector<int> DetectionFusionManager::applyNMS(const std::vector<FusedDetection> &detections)
{
    ...
    // 按置信度排序
    std::vector<std::pair<float, int>> confidences_with_indices; // 存储每个检测框的置信度和索引
    for (size_t i = 0; i < detections.size(); i++)
    {
        confidences_with_indices.push_back({detections[i].confidence, i});
    }

    std::sort(confidences_with_indices.begin(), confidences_with_indices.end(), // 按置信度排序
              [](const std::pair<float, int> &a, const std::pair<float, int> &b)
              {
                  return a.first > b.first;
              });

    std::vector<int> keep_indices;                          // 存储最终保留的检测框索引
    std::vector<bool> suppressed(detections.size(), false); // 用于标记每个检测框是否已被抑制
