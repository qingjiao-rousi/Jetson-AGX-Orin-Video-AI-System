...
usage() {
    cat <<'USAGE'
Usage:
  scripts/run_rtsp_acceptance.sh [INPUT_VIDEO_DIR] [OUTPUT_DIR]
...
Examples:
  scripts/run_rtsp_acceptance.sh

  OUTPUT_SINK=fake RUN_SECONDS=40 START_UI=0 \
    scripts/run_rtsp_acceptance.sh "$VIDEO_DIR" "$OUTPUT_ROOT/rtsp_acceptance_fake"
USAGE
}
...
if [ "$CHECK_RECOVERY" = "1" ]; then
    python3 scripts/check_rtsp_recovery.py \
...
python3 scripts/summarize_rtsp_inproc.py "$OUTPUT_DIR" "$SUMMARY_PATH" \
...
python3 scripts/check_rtsp_inproc_outputs.py "${quality_args[@]}"
quality_exit=$?
...
if [ "$START_UI" = "1" ]; then
    ...
    echo "python3 scripts/preview_web.py --host $UI_HOST --port $UI_PORT --rtsp-dir $OUTPUT_DIR --multifile-dir $OUTPUT_DIR"
fi
