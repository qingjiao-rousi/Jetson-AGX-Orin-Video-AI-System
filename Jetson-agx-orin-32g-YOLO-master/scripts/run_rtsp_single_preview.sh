...
usage() {
    cat <<'USAGE'
Usage:
  scripts/run_rtsp_single_preview.sh [INPUT_VIDEO_OR_DIR] [OUTPUT_DIR]
...
Examples:
  scripts/run_rtsp_single_preview.sh "$VIDEO_DIR"

  SIM_TRANSCODE=1 scripts/run_rtsp_single_preview.sh \
    "$VIDEO_DIR/1.mp4" \
...
scripts/simulate_cameras.sh "$INPUT" --limit 1
